...
public class BigEntPlatformCondition implements Condition {
    ...
    public boolean matches(ConditionContext conditionContext, AnnotatedTypeMetadata annotatedTypeMetadata) {
        ...
        if(!environment.containsProperty("ccod.platformType") || !environment.containsProperty("db.business.count") || !environment.containsProperty("db.business.type"))
            return false;
        else if(CCODPlatformType.BIG_ENT.name.equals(environment.getProperty("ccod.platformType")) && "1".equals(environment.getProperty("db.business.count")) && DBType.ORACLE.name.equals(environment.getProperty("db.business.type")))
            return true;
        return false;
    }
}
